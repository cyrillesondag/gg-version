...
func TestCurrentCmd(t *testing.T) {
	t.Run("HeadTagged", func(t *testing.T) {
		...
		out := captureOutput(t, func() {
			cmdErr = currentCmd(ctx, cmd)
		})
		if cmdErr != nil {
			t.Fatalf("currentCmd error: %v", cmdErr)
		}
	...
	t.Run("HeadNotTagged", func(t *testing.T) {
		...
		// HEAD not tagged → empty output (current only shows tags set on HEAD)
		if strings.TrimSpace(out) != "" {
			t.Errorf("expected empty output for untagged HEAD, got: %q", out)
		}
	})
	...
	t.Run("FormatJSON", func(t *testing.T) {
		...
		if !strings.Contains(out, "2.0.0") {
			t.Errorf("expected '2.0.0' in JSON output, got: %q", out)
		}
	})
}
